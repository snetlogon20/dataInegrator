...
ratings=pd.read_csv(r'C:\Users\ASUS\Downloads\ml-100k1\ml-100k\u.data',sep='\t',names=heads)
print(ratings)
print("用户数量",len(ratings))

import  pandas as pd
u_cols=['user_id','age','sex','occupation','zip_code']
users=pd.read_csv(r'C:\Users\ASUS\Downloads\ml-100k1\ml-100k\u.data',sep='|',names=u_cols,encoding='latin-1')
print(users)
r_cols=['user_id','movie_id','rating','unix_timestamp']
ratings=pd.read_csv(r'C:\Users\ASUS\Downloads\ml-100k1\ml-100k\u.data',sep='\t',names=r_cols,encoding='latin-1')
print(ratings)
m_cols=['movie_id','title','release_data','video_release_data','imdb_url']
movies=pd.read_csv(r'C:\Users\ASUS\Downloads\ml-100k1\ml-100k\u.item',sep='|',names=m_cols,usecols=range(5),encoding='latin-1')
print(movies)
...
print('step 1 读取数据')
r_cols = ['user_id', 'movie_id', 'rating', 'unix_timestamp']
scoredata = pd.read_csv(r'C:\Users\ASUS\Downloads\ml-100k1\ml-100k\u.data', sep='\t', names=r_cols, encoding='latin-1')
...
dataByMovieId=scoredata.groupby('movie_id')
print(len(dataByMovieId))

# n_users = 943
# n_items = 1682

n_users = len(dataByUserId)
n_items = len(dataByMovieId)
